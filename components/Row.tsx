...
import { Thumbanail } from 'components/Thumbanail'
...
export const Row = ({ title, movie }: Props) => {
  ...
  const handleClick = (direction: string) => {
    ...
    console.log({ clientWidth: rowRef.current!.clientWidth, scrollLeft: rowRef.current!.scrollLeft })
  }
  return (
    <div className='h-40 space-y-0.5 md:space-y-2'>
      ...
      <div className='group relative md:-ml-2'>
        ...
        >
          {movie.map((movie) => (
            <Thumbanail key={movie.id} movie={movie} />
          ))}
        </div>
        <ChevronRightIcon
